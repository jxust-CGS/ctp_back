...
public class MD5
{
		...
		//自己加的方法
		public static String getMD5Str2(String password) {//防止被md5字典破解密文，对加密过程加工（我也是够无聊的）呵呵
			MD5 md5 = new MD5();
			String password2 = null;
			char kk=md5.getMD5ofStr(password).charAt(5);
			int j=kk-'!';
			for(int i=0;i<=j%5+1;i++)
			{
				password2=md5.getMD5ofStr(password);
				password=password2+password2.charAt(i);
			}
			//System.out.println(j+" "+(j%5+1));
